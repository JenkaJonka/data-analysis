...
summary(data) 
hist(data$weight.loss) #на общей гистограмме выбросов не видно, разделим на группы по виду диеты
...
out.A <- boxplot.stats(data.A$weight.loss)$out #выбросы по каждой группе
...
boxplot(data.A$weight.loss) #на диаграмме видны выбросы в А-группе
boxplot(data.B$weight.loss) #здесь выбросов нет
boxplot(data.C$weight.loss) #здесь выбросов нет

#выбросы есть только в А-группе, удалим их:
...
boxplot(fixeddata.A$weight.loss) #диаграмма А-группы без выбросов
...
boxplot(weight.loss~diet.type,data=fixeddata,col="light gray",
        ylab = "Weight loss (kg)", xlab = "Diet type")
abline(h=0,col="green")
#без выбросов потеря веса при А-диете меньше, чем с выбросами
